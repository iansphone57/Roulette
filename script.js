...
function updateTrendsAndCoverage() {
    ...
    trendOut.innerHTML = spins.length < 50
        ? "No stable trend yet"
        : "Tracking trends...";

    // 👉 USE LAST 17 SPINS (REALISTIC WINDOW)
    const targets = spins.slice(-17);

    if (targets.length === 0) {
        covOut.innerHTML = "Enter spins to see puck coverage";
        return;
    }
    ...
    if (!result || result.placements.length === 0) {
        covOut.innerHTML = "No solution found";
        return;
    }
